...
ATOM_FEATURES = [SymbolFeature, FormalChargeFeature, DegreeFeature, ExplicitValanceFeature, ImplicitValanceFeature,
                 AromaticityFeature]
...
class MolecularGraph(object):
    ...
    def get_coo_bonds(self):
        n_bonds = 2 * len(
            self.bond_features)  # times 2 since to specify undirected graphs we need to duplicate the edges
    ...
    def to_rdmol(self):
        ...
        with Chem.RWMol() as mol:
            for atom_idx, atom_features in self.atom_features.items():

                symbol_feature = None
                for feature in atom_features:
                    if isinstance(feature, SymbolFeature):
                        symbol_feature = feature

                if symbol_feature is None:
                    raise RuntimeError("No valid SymbolFeature found")

                atom = Chem.Atom(symbol_feature.value)
                new_idx = mol.AddAtom(atom)
                old_idx_to_new_idx[atom_idx] = new_idx
                if hasattr(self, 'atom_weights'):
                    i = self.ordered_atoms[atom_idx]
                    atom_weight = self.atom_weights[i]
                    mol.GetAtomWithIdx(new_idx).SetDoubleProp('weight', atom_weight)

            for (src_atom_idx, dst_atom_idx), bond_features in self.bond_features.items():
                bond_type_feature = None
                for feature in bond_features:
                    if isinstance(feature, BondTypeFeature):
                        bond_type_feature = feature

                if bond_type_feature is None:
                    raise RuntimeError("No valid BondTypeFeature found")

                new_src = old_idx_to_new_idx[src_atom_idx]
                new_dst = old_idx_to_new_idx[dst_atom_idx]

                mol.AddBond(new_src, new_dst, bond_type_feature.value)
        Chem.SanitizeMol(mol)
        return mol

    def add_atom_weights(self, weights):
        """
        Set the atom weights, essentially a sequence of the same size as the number of nodes.
        These weights can be arbitrary things, but are useful for marking atoms for explainability
        """
        self.atom_weights = weights
...
def mols2pickled_graphs(rd_mols, output_dir, atom_features=None, bond_features=None):
    ...
    with multiprocessing.Pool() as pool:
        ...
        for i in tqdm(pool.imap_unordered(mol2pickled_graph, work_packages), desc='Processing graphs',
...
class MolecularGraphDataset(RDKitMolDataset):
    ...
    def from_dataset_spec(cls, *, dataset_spec: DatasetSpec,
        ...
        if dataset_spec.use_cache and datafields_path.exists():
